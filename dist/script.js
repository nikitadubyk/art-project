...
var forms = function forms() {
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      ...
      var name = arr[0].substr(0, 6) + dots + arr[1];
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      postData(api, formData).then(function (res) {
        console.log(res);
        statusImg.setAttribute('src', message.okey);
...
var mask = function mask(selector) {
  var setCurcorPosition = function setCurcorPosition(pos, elem) {
    elem.focus();
...
var showMoreStyles = function showMoreStyles(trigger, card) {
